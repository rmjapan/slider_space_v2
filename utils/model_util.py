...
def load_models_illustrious_xl_(
    pretrained_model_name_or_path: str,
    weight_dtype: torch.dtype = torch.float32,
) -> tuple[
    list[CLIPTokenizer],
    ...
    config = EulerAncestralDiscreteScheduler.load_config(pretrained_model_name_or_path, subfolder="scheduler")
    #想定外のパラメータを削除
    config.pop('clip_sample',None)
    config.pop('interpolation_type',None)
    config.pop('sample_max_value',None)
    config.pop('set_alpha_to_one',None)
    config.pop('skip_prk_steps',None)
    config.pop('use_karras_sigmas',None)
...
def create_noise_scheduler(
    ...
    return scheduler
def load_openvision_model():
    import open_clip
    model,_,image_preprocess = open_clip.create_model_and_transforms('hf-hub:UCSC-VLAA/openvision-vit-large-patch14-224')
...
def load_model_illustrious_xl(params):
    device = params['device']
    weight_dtype = params['weight_dtype']

    
    if params['model_id']=="stabilityai/stable-diffusion-xl-base-1.0":
    ...
    elif params['model_id']=="John6666/any-illustrious-xl-for-lora-training-v01-sdxl":
        tokenizers, text_encoders, unet, noise_scheduler = load_models_illustrious_xl_(
            params['pretrained_model_name_or_path'],
    ...
    if params['encoder'] == 'clip':
        clip_model = CLIPModel.from_pretrained(
            "openai/clip-vit-large-patch14", 
            torch_dtype=weight_dtype
        )
        clip_processor = CLIPProcessor.from_pretrained("openai/clip-vit-large-patch14")
    elif params['encoder'] == 'openvision':
        clip_model,image_preprocess,tokenizer = load_openvision_model()
        clip_processor =[image_preprocess,tokenizer]
        ...
        clip_model = EmotionCLIP(
            video_len=8,
            backbone_checkpoint=None
        )
        print("EmotionCLIP loaded")
        clip_model.load_state_dict(checkpoint['model'], strict=True)
        print("EmotionCLIP loaded state dict")
        clip_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
    clip_model.eval()
    clip_model.to(device, dtype=weight_dtype)
