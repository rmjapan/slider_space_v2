...
try:
    ...
    try:
        from diffusers.utils import load_or_create_model_card, populate_model_card
    except ImportError:
        # 代替実装
        def load_or_create_model_card(model_id, **kwargs):
            return None
        def populate_model_card(model_card, **kwargs):
            return model_card
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/mnt/nas/rmjapan2000/animins/dreambooth-model",
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            ...
            for example in tqdm(
                ...
                negative = "lowres, (bad), text, error, fewer, extra, missing, worst quality, jpeg artifacts, low quality, watermark, unfinished, displeasing, oldest, early, chromatic aberration, signature, extra digits, artistic error, username, scan, abstract, side view"
                if isinstance(example["prompt"], list):
                    negative = [negative]*len(example["prompt"])


                images = pipeline(prompt=example["prompt"], num_inference_steps=21, guidance_scale=7,negative_prompt=negative,).images
